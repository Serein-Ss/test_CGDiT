...
def build_crystal_graph(crystal, graph_method='crystalnn'):
    ...
    if graph_method == 'crystalnn':
        try:
            ...
            crystalNN_tmp = local_env.CrystalNN(distance_cutoffs=None, x_diff_weight=-1, porous_adjustment=False,
                                                search_cutoff=10)
            crystal_graph = StructureGraph.from_local_env_strategy(
...
def process_one(row, niggli, primitive, graph_method, prop_list, use_space_group=False, tol=0.01):
    ...
    # 兼容无 material_id 列的数据集（如 c2db_51 使用 uid 作为唯一标识）
    mp_id = row['material_id'] if 'material_id' in row.index else row.get('uid', str(row.name))
    result_dict.update({
        'mp_id': mp_id,
        'cif': crystal_str,
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    id_col = 'material_id' if 'material_id' in df.columns else ('uid' if 'uid' in df.columns else None)
    ordered_results = [
        mpid_to_results[df.iloc[idx][id_col] if id_col else str(idx)]
...
def create_line_graph_index(data, threebody_cutoff: float):
    """
    计算三体交互的线图索引 (Line Graph Indices)。
    等效于 DGL 的 create_line_graph(g, threebody_cutoff)。

    Args:
        data: PyG Data 或 DataBatch 对象。必须包含 edge_index。
              如果不包含 bond_dist，会自动调用 get_pbc_distances 计算。
        threebody_cutoff (float): 三体相互作用的截断半径。

    Returns:
        line_edge_index: [2, N_triplets] LongTensor
            row 0: target edge index (index in original edge_index)
            row 1: source edge index (index in original edge_index)
            表示对于三体 (k, i, j) 也就是 k->i 和 j->i，
            我们将 j->i 视为 target edge，k->i 视为 source edge。
    """
    if hasattr(data, "bond_dist") and data.bond_dist is not None:
        bond_dist = data.bond_dist
    else:
        ...
        bond_dist = out["distances"]

    mask = bond_dist <= threebody_cutoff
    original_edge_ids = torch.arange(len(bond_dist), device=bond_dist.device)
    valid_edge_ids = original_edge_ids[mask]

    valid_dst = data.edge_index[1][mask]

    sort_idxs = torch.argsort(valid_dst)
    sorted_dst = valid_dst[sort_idxs]
    sorted_edge_ids = valid_edge_ids[sort_idxs]

    unique_dst, counts = torch.unique(sorted_dst, return_counts=True)

    triplet_list = []
    unique_counts = torch.unique(counts)

    ptr = torch.zeros(len(counts) + 1, dtype=torch.long, device=counts.device)
    ptr[1:] = torch.cumsum(counts, dim=0)

    for d in unique_counts:
        d = d.item()
        if d < 2:
            continue

        group_indices = torch.nonzero(counts == d).squeeze(1)

        starts = ptr[group_indices]

        a = torch.arange(d, device=counts.device)
        x, y = torch.meshgrid(a, a, indexing='ij')
        mask_diag = x != y
        src_local = x[mask_diag]  # source local index
        dst_local = y[mask_diag]  # target local index

        src_indices_flat = (starts.unsqueeze(1) + src_local.unsqueeze(0)).flatten()
        dst_indices_flat = (starts.unsqueeze(1) + dst_local.unsqueeze(0)).flatten()

        edge_src = sorted_edge_ids[src_indices_flat]
        edge_dst = sorted_edge_ids[dst_indices_flat]

        triplet_list.append(torch.stack([edge_dst, edge_src], dim=0))

    if len(triplet_list) > 0:
        line_edge_index = torch.cat(triplet_list, dim=1)
    else:
        line_edge_index = torch.empty((2, 0), dtype=torch.long, device=data.edge_index.device)
...
def compute_theta_and_phi(
    ...
    """
    Calculate bond angle Theta and Phi.

    Args:
        bond_vec: [E, 3] 所有边的向量
        bond_dist: [E] 所有边的长度
        line_edge_index: [2, K] 三体索引
            row 0: dst edge index (vec2)
            row 1: src edge index (vec1)
        directed: 是否有向 (default False)
        eps: 数值稳定性 epsilon

    Returns:
        angles: dict containing 'theta' (radians), 'cos_theta', 'phi', 'triple_bond_lengths'
    """
    dst_idx = line_edge_index[0]
    src_idx = line_edge_index[1]

    vec1 = bond_vec[src_idx]
    vec2 = bond_vec[dst_idx]

    if directed:
        # 这里的逻辑需要根据实际 directed graph 的构建方式调整
        # 在 DGL 中是 * edges.src["src_bond_sign"]
        # 简单起见，如果 directed=True，这里假设反向
        vec1 = -vec1

    # 计算 Cosine Theta
    norm_v1 = torch.norm(vec1, dim=1)
    norm_v2 = torch.norm(vec2, dim=1)
    cosine = torch.sum(vec1 * vec2, dim=1) / (norm_v1 * norm_v2 + eps)

    # Stability clamp: [-1+eps, 1-eps]
    cosine = cosine.clamp_(min=-1 + eps, max=1 - eps)
    theta = torch.acos(cosine)
    phi = torch.zeros_like(cosine)
    triple_bond_lengths = bond_dist[dst_idx]

    return {
        "theta": theta,  # Radians
        "cos_theta": cosine,  # Cosine value
        "phi": phi,
        "triple_bond_lengths": triple_bond_lengths  # Length of the target bond
    }
