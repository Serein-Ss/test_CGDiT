...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate generated crystal stability and novelty")
    parser.add_argument("--input", required=True, help="Generated eval_gen_*.pt file")
    parser.add_argument("--train-csv", default="", help="Reference CSV containing a cif column")
    parser.add_argument("--output-dir", default="", help="Report directory; defaults to <input-dir>/Stability")
    parser.add_argument("--relax", action="store_true", help="Relax structures before stability evaluation")
    parser.add_argument("--samples-per-family", type=int, default=3,
                        help="Maximum binary and ternary structures used for expensive stability calculations")
    parser.add_argument("--mlip-model", default="TensorNet-MatPES-r2SCAN-v2025.1-PES")
    ...
    ENABLE_RELAXATION = args.relax
    # ===================================================

    PT_FILE_PATH = str(Path(args.input).resolve())
    MP_API_KEY = os.getenv("MP_API_KEY")

    TRAIN_CSV_PATH = str(Path(args.train_csv).resolve()) if args.train_csv else ""
    TRAIN_STRUCTS = []
    ...
    out_dir = str(Path(args.output_dir).resolve()) if args.output_dir else os.path.join(base_dir, "Stability")
    ...
    evaluator = CrystalEvaluator(mp_api_key=MP_API_KEY, mlip_model_name=args.mlip_model)
    ...
    with open(report_txt, "w", encoding="utf-8") as f:
        ...
        sampled_binaries = binary_structs[:args.samples_per_family]
        sampled_ternaries = ternary_structs[:args.samples_per_family]
