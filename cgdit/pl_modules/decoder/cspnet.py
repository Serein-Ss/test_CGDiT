...
from cgdit.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, \
...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.smooth = smooth
        self.ip = ip
        # 确保没有使用 smooth，因为它与离散的 embedding 不兼容
        if self.smooth:
            # self.node_embedding = nn.Linear(max_atoms, hidden_dim)
            raise ValueError("smooth=True is not compatible with discrete atom type diffusion.")
        else:
        ...
        if self.pred_type:
            self.type_out = nn.Linear(hidden_dim_before_out, MAX_ATOMIC_NUM)
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        if self.smooth:
            ...
            node_features = self.node_embedding(atom_types) # atom_types - 1
