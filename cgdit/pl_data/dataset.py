...
from cgdit.common.utils import PROJECT_ROOT
...
class CrystDataset(Dataset):
    def __init__(self, name: ValueNode, path: ValueNode,
        ...
        self.prop = prop
        # prop_list: 所有要缓存的属性列（决定 pt 文件内容）。
        # 若未指定则仅缓存主属性。建议数据集 yaml 里填写所有可用属性，
        # 这样同一个 pt 文件可被不同条件的实验共享，避免重复预处理。
        self.prop_list = list(prop_list) if prop_list else [prop]
        self.niggli = niggli
    ...
    def __getitem__(self, index):
        ...
        # 将所有缓存的属性都设置为 batch 属性，支持多属性条件生成。
        # self.prop 对应的值额外写入 data.y 作为主训练标签。
        for p in self.prop_list:
            if p in data_dict:
                val = data_dict[p]
                t = torch.tensor([val], dtype=torch.float) if isinstance(val, (float, int)) \
                    else torch.tensor(val, dtype=torch.float)
                setattr(data, p, t)
                if p == self.prop:
                    data.y = t
...
def main(cfg: omegaconf.DictConfig):
    ...
    from cgdit.common.data_utils import get_scaler_from_data_list
