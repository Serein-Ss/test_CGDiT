...
> **对称性约化的连续—离散混合动作策略优化，加上不确定性感知的多保真物理奖励。**

暂定方法描述：

> 在原始晶体空间而不是统一连续潜空间中，为晶格、周期分数坐标和离散原子类型构造对称性约化且可计算的联合策略概率；通过分通道、分时间步信用分配进行 GRPO/PPO 更新，并利用代理模型不确定性、MLIP 松弛和少量 DFT 构成多保真奖励闭环。
...
5. 再接入不确定性奖励和 MLIP/DFT 多保真闭环；
6. 最后开展完整基线、消融、泛化和物理验证。
...
| 日期 | 决策 | 理由 | 影响文件/实验 | 状态 |
|---|---|---|---|---|
| 2026-08-04 | 建议以 symmetry-reduced heterogeneous GRPO 为主线 | 普通晶体 GRPO、多目标奖励和形成能反馈已有直接工作 | 待新增 RL sampler、log-prob、reward 和配置 | 待确认 |
