...
from cgdit.rl.pipo import pipo_feedback
...
def parse_args() -> argparse.Namespace:
    ...
    if args.pipo and args.resume is not None:
        parser.error("PIPO resume is unsupported because reward history is not checkpointed")
...
def _optimize_policy_epochs(
    *,
    ...
    for policy_epoch in range(policy_epochs):
        ...
        weight = 1.0 / len(transition_indices)
        ...
        for transition_index in transition_indices:
            chunk = objective(
                ...
                step_lr=step_lr,
                rewards=rewards if objective.algorithm == "grpo" else None,
                advantages=advantages,
                group_index=group_index if objective.algorithm == "grpo" else None,
                guidance_scale=0.0,
...
def _evaluate_final_policy(
    *,
    ...
    decision = decide_reward_improvement(
        ...
        old_safety_metrics={"validity": old_eval.valid.float()},
        new_safety_metrics={"validity": new_eval.valid.float()},
        safety_tolerances={
            "validity": float(contract.closed_loop.safety_drop_tolerance)
        },
        confidence=float(contract.closed_loop.confidence),
        n_bootstrap=int(contract.closed_loop.bootstrap_samples),
        attenuation=float(contract.closed_loop.attenuation_scale),
...
def main() -> None:
    ...
    scheduled_prompt_count = args.num_prompts * (start_step + args.updates)
    training_prompt_schedule = _prompt_indices(
        len(training_dataset), scheduled_prompt_count, args.seed
    )
    ...
    for step in range(start_step, start_step + args.updates):
        prompt_start = step * args.num_prompts
        ...
        if args.pipo:
            feedback = pipo_feedback(
                ...
                history=reward_history[-args.pipo_history_window :],
                negative_scale=args.pipo_negative_scale,
            )
            pipo_record = {
                "history": list(reward_history[-args.pipo_history_window :]),
                "feedback": None,
            ...
            if feedback is not None:
                ...
                if (
                    ...
                    retrospective_advantages = (
                        previous_replay["advantages"] * feedback.modulation
                    )
                    (
                        _,
                        retrospective_loss,
