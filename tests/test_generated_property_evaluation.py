...
def test_property_metrics_include_single_and_joint_hit_rates():
    ...
    metrics = compute_property_metrics(
        ...
        reference_predictions={name: np.array([0.0, 1.0]) for name in predictions},
        reference_targets={name: np.array([0.0, 1.0]) for name in predictions},
    )

    fe = metrics["properties"]["formation_energy_per_atom"]
    assert fe["n_hits"] == 1
    assert fe["hit_rate_predicted"] == pytest.approx(0.5)
    assert fe["hit_rate_all"] == pytest.approx(1 / 3)
    ...
    assert "wdist_to_test_predictions" in fe


def test_discovery_excludes_pilot_and_smoke_files(tmp_path):
    formal_template = tmp_path / "eval_gen_template_uncond_n4096_seed42.pt"
    formal_abinitio = tmp_path / "eval_gen_abinitio_empirical_uncond_n4096_seed42.pt"
    ...
    assert discover_formal_generation_files(tmp_path) == [formal_abinitio, formal_template]
